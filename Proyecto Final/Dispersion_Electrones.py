...
muestras=100,1000,10000#electrones
ticks=[str(i) for i in muestras]
print(ticks)
F = (2, 5, 8, 12, 15, 20)
Pd=.5
pasomax=1
...
for n in muestras:
    CV=[]
    for Fuerza in F: #tamaño del atomo 
        PORC=[]
...
res_atm = stats.kruskal(N_data[0], N_data[1], N_data[2])
res_elect = stats.kruskal(F1,F2,F3,F4,F5,F6)
...
plt.xlabel('Cantidad de electrones', fontweight ='bold', fontsize = 19)
plt.ylabel('Promedio de dispersiones (%)', fontweight ='bold', fontsize = 19)
plt.xticks([r + barWidth for r in range(len(F1))],
        ticks)
plt.xticks(fontsize = 19)
plt.yticks(fontsize = 19)
plt.legend(title='Tamaño de atomo',fontsize = 19)
